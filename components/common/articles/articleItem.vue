<template>
  <div class="article-list w-full">
    <nuxt-link :to="{ name: 'article-id', params: { id: article.id } }">
      <div
        class="articleItem select-none cursor-pointer overflow-hidden opacity-0 relative hover:shadow-lg bg-gray-100 rounded-lg"
      >
        <div class="star" v-if="article.star == 2">精选</div>
        <!-- 分类 -->
        <span class="category text-gray-900 text-sm block mb-1">{{
        ...
        <div class="others flex justify-between items-center">
          <div class="author-wrapper flex justify-center items-center">
            <i
              class="avatar inline-block rounded-full bg-cover bg-no-repeat shadow-md"
              :style="{ backgroundImage: `url(${article.authors[0].avatar})` }"
            ></i>
            <span class="author-name ml-3 whitespace-nowrap">{{
              article.authors[0].name
            }}</span>
          </div>
          <div class="view flex items-center">
            <span role="img" class="watch"
              ><svg
                t="1654927254929"
                class="icon"
...
export default {
  ...
  data() {
    return {
      loadingPic: "https://xblog-pic-1305549463.cos.ap-guangzhou.myqcloud.com/R-C.gif",
    };
  },
  methods: {
    initImg() {
